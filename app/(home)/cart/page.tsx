...
import { CartItemDescription, CartItemDescriptionProps, CartItemPrice, CartItemPriceProps, InputQuantity } from '../../../components/layout/Header/components/Cart/CartItem';
    ...
    const [cartItems] = useState<CartItemProps[]>([
        {
            ...
            quantity: 2,
            onQuantityChange: (newQuantity: number) => { console.log("Quantity changed to:", newQuantity); },
    ...
    return (
        <Container>
            <div className="flex flex-col w-full">
                ...
                <h2 className={cn("text-5xl text-neutral-900 font-bold", "mt-4")}>My Cart</h2>
                <div className="mt-10 flex flex-col-reverse md:flex-row gap-8 w-full">
                    <div className="flex flex-1 flex-col w-full gap-6">
                        <div className="flex justify-between items-center p-4 border border-neutral-200 rounded-lg">
                            <div><input type="checkbox" name="Select All" id="select-all" /> <label htmlFor="select-all">Select All</label> </div>
                            <Button>Delete</Button>
                        </div>
                        <div className="flex flex-col gap-4">
                            { cartItems.length > 0 ? cartItems.map((item, index) => <CartItem key={index} {...item} />) : <EmptyCart /> }
                        </div>
                    </div>
                    <SummaryOrder />
                </div>
                    ...
                    <ol className="flex gap-1 items-center">
                        {items.map((crumb, index) => (
                            <li key={index+crumb.label} className="text-sm font-medium text-neutral-600">
                                <Link href={crumb.href} className={index === items.length - 1 ? "font-bold text-neutral-900" : ""}>{crumb.label}</Link> { index < items.length - 1 && "/" }
                            </li>
                        ))}
                    </ol>
                </nav>
    ...
    return (
        <div className="flex gap-4 p-4 border border-neutral-200 rounded-lg items-center">
            ...
            <div className="flex flex-col flex-1 h-full pb-2">
                {badge && <Badge as="span" className="mb-2">{badge}</Badge>}
                <Link href={`product?handle=${handle}`} className="text-lg font-bold text-neutral-900 line-clamp-1 mt-auto">{title}</Link>
                <CartItemDescription items={descriptionItems} className="mt-1" />
            </div>
            <div className="flex flex-col items-end justify-between h-full">
                <CartItemPrice currentPrice={price.currentPrice} oldPrice={price.oldPrice} />
                <div className="flex gap-4 items-center">
                    <button className="hover:text-red-600 transition-colors duration-200" onClick={onDelete} aria-label="Delete Item">
